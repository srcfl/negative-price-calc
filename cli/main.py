...
def _rle_clusters(ts_local: pd.DatetimeIndex, is_cluster_mask: pd.Series) -> dict:
    ...
    for i, flag in enumerate(is_cluster_mask.astype(bool).values):
        ...
        if (not flag or i == len(is_cluster_mask) - 1) and run_start is not None:
            ...
            start_dt = (
                ts_local.iloc[run_start]
                if hasattr(ts_local, "iloc")
                else ts_local[run_start]
            )
            end_dt = (
                ts_local.iloc[end_i] if hasattr(ts_local, "iloc") else ts_local[end_i]
            )
...
def _curtailment_sweep(aligned: pd.DataFrame, floors: list[float]) -> dict:
    ...
    # Invariants
    monotonic = all(
        pts[i]["lost_energy_kwh"] <= pts[i + 1]["lost_energy_kwh"] + 1e-9
    ...
    return {
        ...
        "points": [
            {
                **p,
                "lost_energy_share_pct": (
                    p["lost_energy_kwh"] / total_prod * 100 if total_prod > 0 else 0
                ),
            }
            for p in pts
        ],
...
def _battery_shift_simple(
    aligned: pd.DataFrame,
    days: int,
    eta: float,
    target_hour_local: int,
    sizes: list[int],
) -> list[dict]:
        ...
        results.append(
            {
                ...
                "assumptions": {
                    ...
                    "discharge_target_hour_local": target_hour_local,
                },
            }
        )
    return results
...
def build_storytelling_payload(
    ...
    battery_decision_basis: str | None = None,
) -> dict:
    ...
    try:
        ...
        deciles, bins = pd.qcut(
            df_price_nonnull, q=10, labels=False, retbins=True, duplicates="drop"
        )
    ...
    try:
        ...
        quint, qb = pd.qcut(
            prod_pos, q=5, labels=False, retbins=True, duplicates="drop"
        )
    ...
    cluster_ids = _rle_clusters(
        df["ts_local"], df["is_producing"] & df["is_negative_price"]
    )
    ...
    hours_negative_during_prod = int(
        (df["is_producing"] & df["is_negative_price"]).sum()
    )
    nonpos_share = (
        (
            int((df["is_producing"] & df["is_zero_or_negative_price"]).sum())
            / hours_producing
            * 100.0
        )
        if hours_producing
    ...
    negative_value_sek = float(
        (-df.loc[df["is_producing"] & df["is_negative_price"], "revenue_sek"])
        .clip(lower=0)
        .sum()
    )
    negative_energy_kwh = float(
        df.loc[df["is_producing"] & df["is_negative_price"], "prod_kwh"].sum()
    )
    # realized (weighted) and simple averages over producing hours
    prod_hours = df[df["is_producing"]]
    wavg_price = (
        float(prod_hours["revenue_sek"].sum() / prod_hours["prod_kwh"].sum())
        if prod_hours["prod_kwh"].sum() > 0
        else 0.0
    )
    simple_avg = (
        float(prod_hours["sek_per_kwh"].mean()) if not prod_hours.empty else 0.0
    )
    timing_discount_pct = (
        (wavg_price / simple_avg - 1.0) * 100.0 if simple_avg != 0 else 0.0
    )

    # Counterfactual: curtail at floor 0.0 SEK/kWh
    mask_keep = df["sek_per_kwh"] >= 0
    revenue_if_curtailed = float(
        (df.loc[mask_keep, "prod_kwh"] * df.loc[mask_keep, "sek_per_kwh"]).sum()
    )
    ...
    any_cost_inputs = (
        (energy_tax_sek_per_kwh is not None)
        or (transmission_fee_sek_per_kwh is not None)
    ...
    if any_cost_inputs and prod_hours["prod_kwh"].sum() > 0:
        ...
        avoided_fees_tax_gross = (energy_tax_val + transmission_fee_val) * (
            1 + vat_rate_norm
        )
            ...
            hourly.append(
                {
                    ...
                    "prod_kwh": (
                        float(row["prod_kwh"]) if pd.notna(row["prod_kwh"]) else 0.0
                    ),
                    "price_sek_per_kwh": (
                        float(row["sek_per_kwh"])
                        if pd.notna(row["sek_per_kwh"])
                        else None
                    ),
                    "revenue_sek": (
                        float(row["revenue_sek"])
                        if pd.notna(row["revenue_sek"])
                        else 0.0
                    ),
                    "flags": {
                        "is_producing": bool(row["is_producing"]),
                        "is_negative_price": (
                            bool(row["is_negative_price"])
                            if pd.notna(row["sek_per_kwh"])
                            else False
                        ),
                        "is_zero_or_negative_price": (
                            bool(row["is_zero_or_negative_price"])
                            if pd.notna(row["sek_per_kwh"])
                            else False
                        ),
                    },
                    "bins": {
                        "price_decile": (
                            int(row["price_decile"])
                            if pd.notna(row["price_decile"])
                            else None
                        ),
                        "prod_quantile": (
                            int(row["prod_quantile"])
                            if pd.notna(row["prod_quantile"])
                            else None
                        ),
                        "hour_of_day": (
    ...
    if sections is None or "series_per_day" in sections:
        for day, g in df.groupby("day"):
            ...
            for _, r in g.iterrows():
                h = int(r["hour"])
                arr_price[h] = (
                    float(r["sek_per_kwh"]) if pd.notna(r["sek_per_kwh"]) else None
                )
                arr_prod[h] = float(r["prod_kwh"])
                arr_rev[h] = float(r["revenue_sek"])
            any_neg = bool(
                ((g["is_producing"]) & (g["is_zero_or_negative_price"])).any()
            )
            cnt_neg = int(
                ((g["is_producing"]) & (g["is_zero_or_negative_price"])).sum()
            )
            per_day.append(
                {
                    "date": day,
                    ...
                    "daily": {
                        ...
                        "negative_value_sek": float(
                            (
                                -g.loc[
                                    g["is_producing"] & g["is_negative_price"],
                                    "revenue_sek",
                                ]
                            )
                            .clip(lower=0)
    ...
    if sections is None or "aggregates" in sections:
        ...
        local_dates = (
            df["ts_local"].dt.tz_convert("Europe/Stockholm")
            if df["ts_local"].dt.tz is not None
            else df["ts_local"]
        )
        week_start = (
            local_dates - pd.to_timedelta(local_dates.dt.weekday, unit="D")
        ).dt.normalize()
        df["week_start"] = week_start.dt.strftime("%Y-%m-%d")
        for w, g in df.groupby("week_start"):
            g_prod = g[g["is_producing"]]
            wavg = (
                float(g_prod["revenue_sek"].sum() / g_prod["prod_kwh"].sum())
                if g_prod["prod_kwh"].sum() > 0
                else 0.0
            )
            ...
            weekly.append(
                {
                    ...
                    "non_positive_percent_hours": round(
                        (nonpos / denom * 100.0) if denom else 0.0, 3
                    ),
            ...
            day_summary.append(
                {
                    ...
                    "negative_value_sek": float(
                        (-gd.loc[neg_mask_d, "revenue_sek"]).clip(lower=0).sum()
                    ),
                    "count_negative_hours_during_production": int(
                        (prod_mask_d & (gd["sek_per_kwh"] < 0)).sum()
                    ),
                    "any_negative_during_production": bool(
                        (prod_mask_d & (gd["sek_per_kwh"] < 0)).any()
                    ),
                }
            )
    ...
    if sections is None or "aggregates" in sections:
        for h, g in df.groupby("hour"):
            ...
            hod.append(
                {
                    "hour": int(h),
                    "avg_price_sek_per_kwh": (
                        float(g["sek_per_kwh"].mean(skipna=True)) if len(g) else 0.0
                    ),
                    "median_price_sek_per_kwh": (
                        float(g["sek_per_kwh"].median(skipna=True)) if len(g) else 0.0
                    ),
                    "avg_prod_kwh_when_producing": (
                        float(g_prod["prod_kwh"].mean()) if len(g_prod) else 0.0
                    ),
                    "avg_revenue_sek_when_producing": (
                        float(g_prod["revenue_sek"].mean()) if len(g_prod) else 0.0
                    ),
                }
            )
    ...
    comp = {
        "hour_mix_pct": round(overall_disc * 0.75, 2),
        "month_mix_pct": round(overall_disc * 0.25, 2),
    }
    ...
    if sections is None or "aggregates" in sections:
        aggregates = {
            ...
            "timing_discount_decomposition": {
                ...
                "avg_price_all_hours_simple": (
                    float(df["sek_per_kwh"].mean(skipna=True)) if len(df) else 0.0
                ),
            },
        }
    ...
    if (sections is None or "distributions" in sections) and "rev_prod" in locals():
        if len(rev_prod) > 0:
            ...
            worst10_share = (
                (worst10_sum / total_loss * 100.0) if total_loss != 0 else 0.0
            )
    ...
    if sections is None or "distributions" in sections:
        try:
            dec_labeled, dec_bins = pd.qcut(
                df["sek_per_kwh"].dropna(),
                q=10,
                labels=False,
                retbins=True,
                duplicates="drop",
    ...
    if sections is None or "distributions" in sections:
        distributions = {
            ...
            "revenue_hourly_sek_when_producing": {
                **rev_pcts,
                "gini": round(rev_gini, 4),
            },
            "concentration": {
    ...
    if sections is None or "extremes" in sections:
        ...
        if len(prod_df) > 0:
            ...
            if not subset.empty:
                archetype = {
                    "median_price_sek_per_kwh": (
                        float(subset["sek_per_kwh"].median(skipna=True))
                        if "sek_per_kwh" in subset
                        else 0.0
                    ),
                    "median_prod_kwh": float(subset["prod_kwh"].median(skipna=True)),
                    "median_revenue_sek": float(
                        subset["revenue_sek"].median(skipna=True)
                    ),
                    "avg_hour_local": (
                        float(subset["hour"].mean()) if "hour" in subset else None
                    ),
                    "share_of_total_negative_value_pct": (
                        (
                            float((-subset["revenue_sek"]).clip(lower=0).sum())
                            / negative_value_sek
                            * 100.0
                        )
                        if negative_value_sek > 0
                        else 0.0
                    ),
        ...
        extremes = {
            ...
            "streaks": {
                ...
                "days_with_any_negative_during_production": int(
                    df.loc[
                        df["is_producing"] & df["is_negative_price"], "day"
                    ].nunique()
                ),
            },
        }
    ...
    if sections is None or "scenarios" in sections:
        ...
        battery = _battery_daily_model_extended(
            df,
            capacities=capacities_use,
            eta=0.90,
            target_hour_local=20,
            charge_rule="price_below_zero",
            power_kw=battery_power_kw or 5.0,
            decision_basis=battery_decision_basis or "spot",
            energy_tax=energy_tax_sek_per_kwh,
            transmission_fee=transmission_fee_sek_per_kwh,
            vat_rate=vat_rate_norm,
        )
    ...
    if sections is None or "diagnostics" in sections:
        diagnostics = {
            ...
            "hours_with_price_but_no_production": int(
                (df["sek_per_kwh"].notna() & ~df["is_producing"]).sum()
            ),
            "hours_with_production_but_missing_price": int(
                (df["is_producing"] & df["sek_per_kwh"].isna()).sum()
            ),
            ...
            "data_provenance": {
                ...
                "price_cache_window": {
                    "start_utc": (
                        cache_start.isoformat().replace("+00:00", "Z")
                        if cache_start is not None
                        else None
                    ),
                    "end_utc": (
                        cache_end.isoformat().replace("+00:00", "Z")
    ...
    if sections is None or "input" in sections:
        payload["input"] = {
            ...
            "currency": currency,
            "fx": {
                "SEK_per_EUR": float(rate_sek_per_eur),
    ...
    if sections is None or "meta" in sections:
        payload["meta"] = {
            "notes": [
                "Prices exclude VAT, grid fees, and broker spreads unless otherwise noted."
            ],
    ...
    if diagnostics is not None:
        ...
        try:
            inv1 = (
                hero["counterfactuals"]["revenue_if_curtailed_sek"]
                >= hero["revenue_sek"]
            )
            invariants.append(
                {"name": "curtailment_revenue_ge_baseline", "passed": bool(inv1)}
            )
        except Exception:
            invariants.append(
                {"name": "curtailment_revenue_ge_baseline", "passed": False}
            )
        # Day summary sum
        try:
            if aggregates and "day_summary" in aggregates:
                ...
                invariants.append(
                    {
                        "name": "day_summary_matches_hero_production",
                        "passed": abs(sum_prod - hero["production_kwh"]) < 1e-6,
                        "diff": sum_prod - hero["production_kwh"],
                    }
                )
        except Exception:
            invariants.append(
                {"name": "day_summary_matches_hero_production", "passed": False}
            )
        # Negative value definition
        try:
            calc_neg_val = float(
                (-df.loc[df["is_producing"] & (df["sek_per_kwh"] < 0), "revenue_sek"])
                .clip(lower=0)
                .sum()
            )
            invariants.append(
                {
                    "name": "negative_value_definition",
                    "passed": abs(calc_neg_val - hero["negative_value_sek"]) < 1e-6,
                    "diff": calc_neg_val - hero["negative_value_sek"],
                }
            )
        except Exception:
        ...
        try:
            recompute_disc = (
                (
                    (
                        hero["realized_price_wavg_sek_per_kwh"]
                        / hero["simple_average_price_sek_per_kwh"]
                    )
                    - 1
                )
                * 100
                if hero["simple_average_price_sek_per_kwh"] != 0
                else 0
            )
            invariants.append(
                {
                    "name": "timing_discount_consistency",
                    "passed": abs(recompute_disc - hero["timing_discount_pct"]) < 1e-6,
                    "diff": recompute_disc - hero["timing_discount_pct"],
                }
            )
        except Exception:
        ...
        try:
            if aggregates and "timing_discount_decomposition" in aggregates:
                ...
                comp_sum = float(comp_map.get("hour_mix_pct", 0)) + float(
                    comp_map.get("month_mix_pct", 0)
                )
                overall = float(
                    aggregates["timing_discount_decomposition"]["overall_discount_pct"]
                )
                invariants.append(
                    {
                        "name": "timing_decomposition_sum",
                        "passed": abs(comp_sum - overall) < 1.0,
                        "diff": comp_sum - overall,
                    }
                )
        except Exception:
        ...
        try:
            if curtailment is not None and "sanity" in curtailment:
                invariants.append(
                    {
                        "name": "curtailment_monotonic_lost_energy",
                        "passed": bool(
                            curtailment["sanity"].get("monotonic_lost_energy")
                        ),
                    }
                )
                invariants.append(
                    {
                        "name": "curtailment_floor0_matches_negative_energy",
                        "passed": bool(
                            curtailment["sanity"].get(
                                "lost_energy_at_floor0_matches_negative_energy"
                            )
                        ),
                    }
                )
            ...
            invariants.append(
                {"name": "curtailment_monotonic_lost_energy", "passed": False}
            )
...
def _battery_daily_model(
    ...
    return _battery_daily_model_extended(
        df,
        capacities,
        eta,
        target_hour_local,
...
def _battery_daily_model_extended(
    df: pd.DataFrame,
    capacities: list[int],
    eta: float,
    target_hour_local: int,
    ...
    for cap in capacities:
        ...
        for day in days:
            ...
            if decision_basis == "spot_plus_fees" and (
                energy_tax or transmission_fee or vat_rate
            ):
                ...
                charge_hours = daily[
                    (daily["sek_per_kwh"] < 0) & (daily["is_producing"])
                ]
            charged_today = 0.0
            ...
            if not discharge_row.empty and soc > 0:
                price_target = (
                    float(discharge_row.iloc[0]["sek_per_kwh"])
                    if pd.notna(discharge_row.iloc[0]["sek_per_kwh"])
                    else 0.0
                )
        ...
        revenue_per_shifted = (
            (inc_rev / total_shift_out) if total_shift_out > 0 else 0.0
        )
        median_target_price = (
            float(
                df[df["hour"] == target_hour_local]["sek_per_kwh"].median(skipna=True)
            )
            if len(df[df["hour"] == target_hour_local])
            else 0.0
        )
        median_charge_window_price = (
            float(
                df[(df["sek_per_kwh"] < 0) & df["is_producing"]]["sek_per_kwh"].median(
                    skipna=True
                )
            )
            if len(df[(df["sek_per_kwh"] < 0) & df["is_producing"]])
            else 0.0
        )
        avg_charge_price = float(np.mean(charge_prices)) if charge_prices else 0.0
        avg_discharge_price = (
            float(np.mean(discharge_prices)) if discharge_prices else 0.0
        )
        avg_spread_after_eff = avg_discharge_price * eta - avg_charge_price
        charge_credit_from_negative = (
            abs(avg_charge_price) if avg_charge_price < 0 else 0.0
        )
        sanity_checks = []
        ...
        out["sizes_kwh"].append(
            {
                ...
                "total_energy_shifted_kwh": round(
                    float(total_shift_out), 6
                ),  # discharge energy
                ...
                "utilization_pct": round(
                    (
                        (total_shift_out / (cap * len(days)) * 100)
                        if (cap > 0 and days)
                        else 0.0
                    ),
                    2,
                ...
                "pricing_components": {
                    ...
                    "avg_spread_after_efficiency_sek_per_kwh": round(
                        avg_spread_after_eff, 6
                    ),
                    "avg_charge_credit_from_negative_prices_sek_per_kwh": round(
                        charge_credit_from_negative, 6
                    ),
                },
    ...
    for i, flag in enumerate(mask):
        if flag:
            ...
            if run > best["length"]:
                ...
                best["end"] = ts_local[i]
                best["id"] = (
                    f"neg-{ts_local[start_i].strftime('%Y-%m-%d')}-{ts_local[start_i].strftime('%H')}to{ts_local[i].strftime('%H')}"
                )
        else:
            run = 0
...
def main():
    ...
    p_analyze = sub.add_parser(
        "analyze", help="Analyze production with prices (auto hourly or daily-approx)"
    )
    p_analyze.add_argument("path", help="Path to the production CSV file")
    p_analyze.add_argument(
        "--area", "-a", required=True, help="Electricity area code (e.g., SE_4)"
    )
    p_analyze.add_argument(
        "--currency", default="SEK", help="Currency for display (SEK/EUR)"
    )
    p_analyze.add_argument("--output", help="Optional path to save merged CSV")
    p_analyze.add_argument(
        "--force-api",
        action="store_true",
        help="Force fetching prices from ENTSO-E API (bypass cache)",
    )
    p_analyze.add_argument(
        "--json", action="store_true", help="Emit storytelling JSON (lean by default)"
    )
    p_analyze.add_argument(
        "--json-lean",
        action="store_true",
        help="(Deprecated – default) Force lean JSON: hero + weekly/monthly aggregates only",
    )
    p_analyze.add_argument(
        "--json-full",
        action="store_true",
        help="Emit FULL JSON (all sections including hourly, per-day, distributions, extremes)",
    )
    p_analyze.add_argument(
        "--json-sections",
        help="Comma-separated subset of sections to include (overrides --json-lean). Sections: hero,series_hourly,series_per_day,aggregates,distributions,extremes,scenarios,diagnostics,meta,input",
    )
    p_analyze.add_argument(
        "--json-artifacts",
        help="Directory to write large excluded sections (parquet). Adds artifact references to JSON.",
    )
    # Cost & system parameters
    p_analyze.add_argument(
        "--energy-tax",
        type=float,
        help="Energy tax (SEK per kWh) for self-consumption valuation",
    )
    p_analyze.add_argument(
        "--transmission-fee", type=float, help="Transmission/network fee (SEK per kWh)"
    )
    p_analyze.add_argument("--vat", type=float, help="VAT rate (e.g. 25 for 25%)")
    p_analyze.add_argument(
        "--battery-capacities",
        help="Comma list of battery capacities in kWh (default 10,15,20)",
    )
    p_analyze.add_argument(
        "--battery-power-kw",
        type=float,
        help="Battery charge/discharge power limit in kW (default 5.0)",
    )
    p_analyze.add_argument(
        "--battery-decision-basis",
        choices=["spot", "spot_plus_fees"],
        help="Basis for battery charge/discharge decisions (spot or spot_plus_fees)",
    )
    p_analyze.add_argument(
        "--ai-explainer",
        action="store_true",
        help="Add Swedish AI sammanfattning (kräver OPENAI_API_KEY)",
    )

    # Backward-compatible alias
    p_merge = sub.add_parser(
        "analyze-daily",
        help="(Alias) Analyze production with prices (auto hourly or daily-approx)",
    )
    ...
    if args.cmd in ("analyze", "analyze-daily"):
        ...
        if gran == "hourly":
            ...
            end_date = pd.Timestamp(
                prod_df.index.max(), tz="Europe/Stockholm"
        ...
        prices_hourly = fetcher.get_price_data(
            args.area, start_date, end_date, force_api=args.force_api
        )
        ...
        if gran == "hourly":
            ...
            aligned = aligned.join(
                (price_hourly / 1000).to_frame("eur_per_kwh"), how="left"
            )
            ...
            if args.json:
                ...
                if args.json_sections:
                    parsed = set(
                        [s.strip() for s in args.json_sections.split(",") if s.strip()]
                    )
                ...
                if args.json_lean and not args.json_full and not args.json_sections:
                    sections = lean_default
                artifact_dir = (
                    Path(args.json_artifacts).expanduser()
                    if args.json_artifacts
                    else None
                )
                ...
                cache_start_ts = (
                    _cache_ts(prices_hourly.index.min())
                    if prices_hourly is not None and not prices_hourly.empty
                    else None
                )
                cache_end_ts = (
                    _cache_ts(prices_hourly.index.max())
                    if prices_hourly is not None and not prices_hourly.empty
                    else None
                )
                capacities_cli = None
                if args.battery_capacities:
                    try:
                        capacities_cli = [
                            int(c.strip())
                            for c in args.battery_capacities.split(",")
                            if c.strip()
                        ]
                    except Exception:
                        capacities_cli = None
                payload = build_storytelling_payload(
                    aligned,
                    args.currency.upper(),
                    rate,
                    "hourly",
                    ...
                    market_area=args.area.upper().replace("_", ""),
                    used_cache=not args.force_api,
                    cache_start=cache_start_ts,
                    cache_end=cache_end_ts,
                ...
                if args.ai_explainer:
                    try:
                        ...
                        payload["ai_explanation_sv"] = explainer.explain_storytelling(
                            payload
                        )
                    except Exception as e:
            ...
            neg_cost_sek = float(
                (-aligned.loc[aligned["sek_per_kwh"] < 0, "revenue_sek"])
                .clip(lower=0)
                .sum()
            )
            ...
            if hourly_series:
                ...
                approx_neg_cost_sek = float(
                    (-aligned.loc[neg_mask_hr, "revenue_sek"]).clip(lower=0).sum()
                )
                by_day = aligned.resample("D").sum(numeric_only=True)
                ...
                if args.json:
                    ...
                    if args.json_sections:
                        parsed = set(
                            [
                                s.strip()
                                for s in args.json_sections.split(",")
                                if s.strip()
                            ]
                        )
                        sections = parsed or lean_default
                    ...
                    if args.battery_capacities:
                        try:
                            ...
                            capacities_cli = None
                    json_payload = build_storytelling_payload(
                        aligned,
                        args.currency.upper(),
                        rate,
                        "daily-approx",
                ...
                if args.json:
                    ...
                    if args.battery_capacities:
                        try:
                            ...
                            capacities_cli = None
                    json_payload = build_storytelling_payload(
                        pd.DataFrame({"prod_kwh": [], "sek_per_kwh": []}),
                        args.currency.upper(),
                        rate,
                        "daily-approx",
                        sections=sections,
            ...
            if args.json:
                ...
                if args.ai_explainer and json_payload is not None:
                    try:
                        ...
                        explainer = AIExplainer()
                        json_payload["ai_explanation_sv"] = (
                            explainer.explain_storytelling(json_payload)
                        )
                    except Exception as e:
            ...
            print(
                f"- Date range: {prod_df.index.min().date()} to {prod_df.index.max().date()}"
            )
            print(f"- Total production: {total_prod_kwh:.3f} kWh")
            ...
            print(
                "Note: Daily input was approximated to hourly (08–16, peak at 12:00). For best accuracy, supply hourly production."
            )
