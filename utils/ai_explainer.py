...
class AIExplainer:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if "Read timed out" in msg or "Timeout" in msg:
                    ...
                    if any(
                        k in msg
                        for k in ("401", "auth", "quota", "model_not_found", "429")
                    ):
                        break
    ...
    def _facts_to_bullet_line(self, facts: Dict[str, Any]) -> str:
        ...
        if prod_val > 0 and "timmar_som_kostat_dig" in facts:
            bits = [
                f"Årsproduktion {fmt(prod_val)} kWh",
                f"totala intäkter {fmt(rev_val)} SEK",
            ]
        else:
        ...
        if "timmar_som_kostat_dig" in facts:
            bits.append(
                f"{fmt(facts['timmar_som_kostat_dig'])} timmar då export kostade pengar"
            )
        if "kwh_exporterat_med_förlust" in facts:
            bits.append(
                f"{fmt(facts['kwh_exporterat_med_förlust'])} kWh exporterat vid negativa priser"
            )
        if "andel_olönsam_export_pct" in facts:
            bits.append(
                f"{fmt(facts['andel_olönsam_export_pct'])}% av exporten var olönsam"
            )

        # OPTIMIZATION POTENTIAL
        if "zap_negativa_timmar" in facts:
            bits.append(
                f"Exportstyrning kan optimera {fmt(facts['zap_negativa_timmar'])} timmar"
            )
        if "zap_export_optimerad_kwh" in facts:
            bits.append(
                f"Styrning kan optimera {fmt(facts['zap_export_optimerad_kwh'])} kWh export"
            )
        if "zap_dagar_drabbade" in facts:
            bits.append(
                f"Påverkan kan minskas {fmt(facts['zap_dagar_drabbade'])} dagar"
            )
    ...
    def _build_prompt(self, bullet_line: str, facts: Dict[str, Any]) -> str:
        ...
        has_zap_data = (
            "zap_negativa_timmar" in facts or "zap_export_optimerad_kwh" in facts
        )
    ...
    def _manual_fallback(self, facts: Dict[str, Any], reason: str) -> str:
        ...
        try:
            ...
            parts = [
                f"Anläggningen producerade {prod} kWh med totala intäkter {rev} SEK."
            ]
