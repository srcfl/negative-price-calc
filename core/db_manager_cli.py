...
class PriceDatabaseManager:
    ...
    def show_database_info(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Get areas
            areas = conn.execute(
                "SELECT DISTINCT area_code FROM price_data ORDER BY area_code"
            ).fetchall()

            # Get date range
            date_range = conn.execute(
                "SELECT MIN(datetime), MAX(datetime) FROM price_data"
            ).fetchone()
            ...
            if areas:
                ...
                for (area,) in areas:
                    ...
                    print(
                        f"  {area}: {area_info[0]:,} records from {area_info[1]} to {area_info[2]}"
                    )
    ...
    def show_area_info(self, area_code, eur_sek_rate=11.5):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            count = conn.execute(
                "SELECT COUNT(*) FROM price_data WHERE area_code = ?", (area_code,)
            ).fetchone()[0]
            ...
            print(
                f"Price range: {min_price_sek:.4f} to {max_price_sek:.4f} SEK/kWh ({stats[3]:.2f} to {stats[4]:.2f} EUR/MWh)"
            )
            print(
                f"Average price: {avg_price_sek:.4f} SEK/kWh ({stats[5]:.2f} EUR/MWh)"
            )
            ...
            if monthly_data:
                ...
                for month, records, avg_price in monthly_data:
                    ...
                    print(
                        f"  {month}: {records:,} records, avg price: {avg_price_sek:.4f} SEK/kWh ({avg_price:.2f} EUR/MWh)"
                    )
    ...
    def clear_area_data(self, area_code):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Confirm deletion
            response = input(
                f"This will delete {count:,} records for area {area_code}. Continue? (y/N): "
            )
            if response.lower() != "y":
                print("Cancelled.")
                return
...
def main():
    ...
    parser.add_argument(
        "--db-path", default="price_data.db", help="Path to the database file"
    )
    ...
    parser.add_argument(
        "--clear", action="store_true", help="Clear data for specified area"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=11.5,
        help="EUR to SEK exchange rate for display (default: 11.5)",
    )
