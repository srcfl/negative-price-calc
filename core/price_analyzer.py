...
class PriceAnalyzer:
    ...
    def merge_data(self, prices_df, production_df, currency_rate=11.5):
        ...
        merged_df["price_sek_per_kwh"] = (
            merged_df["price_eur_per_mwh"] * currency_rate / 1000
        )
        ...
        logger.info(
            f"Merged data: {len(merged_df)} records from {merged_df.index.min()} to {merged_df.index.max()}"
        )
    ...
    def analyze_data(self, merged_df):
        ...
        analysis["period_days"] = (
            merged_df.index.max() - merged_df.index.min()
        ).days + 1
        ...
        if analysis["negative_price_hours"] > 0:
            ...
            analysis["production_during_negative_prices"] = merged_df.loc[
                negative_production_mask, "production_kwh"
            ].sum()
            analysis["avg_production_during_negative_prices"] = merged_df.loc[
                negative_production_mask, "production_kwh"
            ].mean()
            analysis["negative_export_cost_abs_sek"] = abs(
                merged_df.loc[negative_mask, "export_value_sek"].sum()
            )
        else:
        ...
        analysis["positive_export_value_sek"] = merged_df.loc[
            positive_mask, "export_value_sek"
        ].sum()
...
class PriceAnalyzer:
    ...
    @staticmethod
    def merge_data(
        prices_df: pd.DataFrame, production_df: pd.DataFrame, eur_sek_rate: float = 11.5
    ) -> pd.DataFrame:
        """
        Merge price and production data on datetime index.
    ...
    def analyze_data(merged_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        sample_df = (
            merged_df.head(time_series_limit)
            if len(merged_df) > time_series_limit
            else merged_df
        )
        ...
        daily_data = (
            merged_df.groupby(merged_df.index.date)
            .agg(
                {
                    "production_kwh": "sum",
        ...
        analysis["negative_price_percentage"] = (
            (len(negative_prices) / len(merged_df)) * 100 if len(merged_df) > 0 else 0
        )
        analysis["production_percentage_negative_prices"] = (
            (
                analysis["production_during_negative_prices"]
                / analysis["production_total"]
            )
            * 100
            if analysis["production_total"] > 0
            else 0
        )
        ...
        if len(negative_prices) > 0:
            ...
            analysis["worst_negative_price_eur_mwh"] = negative_prices.loc[
                worst_negative_idx, "price_eur_per_mwh"
            ]
            analysis["worst_negative_price_production"] = negative_prices.loc[
                worst_negative_idx, "production_kwh"
            ]
            analysis["worst_negative_price_cost"] = abs(
                negative_prices.loc[worst_negative_idx, "export_value_sek"]
            )
        else:
            analysis["avg_production_during_negative_prices"] = 0
        ...
        if (
            merged_df["production_kwh"].var() > 0
            and merged_df["price_sek_per_kwh"].var() > 0
        ):
            analysis["price_production_correlation"] = merged_df["production_kwh"].corr(
                merged_df["price_sek_per_kwh"]
            )
        else:
        ...
        analysis["price_volatility_cv"] = (
            analysis["price_volatility_std"] / analysis["price_mean_sek_kwh"]
            if analysis["price_mean_sek_kwh"] != 0
            else 0
        )
    ...
    def print_analysis(analysis: Dict[str, Any]):
        ...
        print(
            f"  Price volatility (std): {analysis['price_volatility_std']:.4f} SEK/kWh"
        )
        ...
        print(f"\nCORRELATION ANALYSIS:")
        print(
            f"  Price-Production correlation: {analysis['price_production_correlation']:.3f}"
        )
