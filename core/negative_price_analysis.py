...
def analyze_negative_pricing(csv_file):
    ...
    print(f"  Hours with negative prices AND production: {len(negative_with_production):,}")
    ...
    print(f"  Lowest price: {negative['price_eur_per_mwh'].min():.2f} EUR/MWh ({negative['price_sek_per_kwh'].min():.4f} SEK/kWh)")
    print(f"  Average negative price: {negative['price_eur_per_mwh'].mean():.2f} EUR/MWh ({negative['price_sek_per_kwh'].mean():.4f} SEK/kWh)")
    
    print(f"\nPRODUCTION DURING NEGATIVE PRICES:")
    print(f"  Total production: {negative['production_kwh'].sum():.2f} kWh")
    print(f"  Average hourly production: {negative_with_production['production_kwh'].mean():.3f} kWh")
    print(f"  Max hourly production: {negative_with_production['production_kwh'].max():.3f} kWh")
    
    print(f"\nCOST ANALYSIS:")
    total_cost = abs(negative['export_value_sek'].sum())
    print(f"  TOTAL COST of negative price exports: {total_cost:.2f} SEK")
    print(f"  Most expensive single hour: {abs(negative['export_value_sek'].min()):.2f} SEK")
    print(f"  Average cost per kWh during negative prices: {total_cost / negative['production_kwh'].sum():.4f} SEK/kWh")
    ...
    monthly = monthly[monthly['production_kwh'] > 0]  # Only months with production during negative prices
    
    for month, row in monthly.iterrows():
        print(f"  {month}: {row['production_kwh']:.1f} kWh exported, cost: {row['cost_sek']:.2f} SEK")
    ...
    for idx, row in top_expensive.iterrows():
        print(f"  {idx}: {row['production_kwh']:.3f} kWh @ {row['price_sek_per_kwh']:.4f} SEK/kWh = {abs(row['export_value_sek']):.3f} SEK")
    ...
    print(f"  Most expensive day: {daily_costs['cost_sek'].max():.2f} SEK on {daily_costs['cost_sek'].idxmax()}")
    ...
    print(f"  Negative pricing reduces income by: {(total_cost/positive_export_value)*100:.2f}%")
