...
class PriceFetcher:
    ...
    def __init__(self, db_path="data/price_data.db"):
        ...
        if not self.api_key:
            logger.warning(
                "No ENTSO-E API key found. Using existing database data only."
            )
    ...
    def get_price_data(self, area_code, start_date, end_date, force_api: bool = False):
        ...
        if not force_api and self.db_manager.has_data_for_period(
            area_code, start_date, end_date
        ):
            logger.info(
                f"Using cached data for {area_code} from {start_date} to {end_date}"
            )
        ...
        if self.client is not None:
            logger.info(
                f"Fetching price data from ENTSO-E API for {area_code} (force_api={force_api})"
            )
            ...
            while chunk_start < chunk_end:
                ...
                try:
                    ...
                    series = self.client.query_day_ahead_prices(
                        zone, chunk_start.tz_convert("UTC"), next_end.tz_convert("UTC")
                    )
                except Exception as e:
                    logger.error(
                        f"ENTSOE client error for chunk {chunk_start} to {next_end}: {e}"
                    )
                    series = None
                if series is not None and not series.empty:
                    ...
                    logger.warning(
                        f"No data returned for chunk {chunk_start} to {next_end}"
                    )
        ...
        logger.warning(
            f"Incomplete data available for {area_code} from {start_date} to {end_date}"
        )
        return self.db_manager.get_price_data(area_code, start_date, end_date)
