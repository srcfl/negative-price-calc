...
class DailyAnalyzer:
    ...
    def merge_daily_data(
        ...
        daily_prices = (
            prices_df.groupby(prices_df.index.date).agg(agg_dict).reset_index()
        )
        ...
        # Merge on date
        merged_df = pd.merge(
            daily_prices, production_df, left_index=True, right_index=True, how="inner"
        )
        ...
        logger.info(
            f"Merged daily data: {len(merged_df)} rows from {merged_df.index.min()} to {merged_df.index.max()}"
        )
    ...
    def analyze_daily_data(merged_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Price distribution
        price_percentiles = merged_df["price_eur_per_mwh"].quantile(
        ...
        analysis = {
            ...
            "value_summary": {
                ...
                "value_per_kwh_sek": float(
                    total_export_value / total_production if total_production > 0 else 0
                ),
            },
            "best_days": {
                "highest_price": {
                ...
                "highest_production": {
                    ...
                    "price_eur_per_mwh": float(
                        best_production_day["price_eur_per_mwh"]
                    ),
                ...
                "lowest_production": {
                    ...
                    "price_eur_per_mwh": float(
                        worst_production_day["price_eur_per_mwh"]
                    ),
        ...
        logger.info(
            f"Daily analysis completed: {total_production:.1f} kWh total production, "
