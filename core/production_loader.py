...
class ProductionLoader:
    ...
    def load_production(
        self, file_path: str, use_llm: bool = False
        ...
        try:
            logger.info(
                "Schema not standard or read failed; invoking AI-assisted parser for: %s",
                file_path,
            )
        ...
        raise ValueError(
            "Unrecognized production file format. Expected headers with Date/Datum and a kWh production column, or enable AI parsing via OPENAI_API_KEY."
        )
    ...
    def _process_auto(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, str]:
        ...
        if not date_col or not prod_col:
            raise ValueError(
                f"Could not detect date/production columns. Columns: {list(df.columns)}"
            )
        ts = self._parse_datetime_series(
            df[date_col], time_series=df[time_col] if time_col else None
        )
        # If nothing parsed to a datetime, fail clearly
        if pd.Series(ts).notna().sum() == 0:
            raise ValueError(
                "No date/datetime values could be parsed; ensure the file contains a Date/Datum column."
            )
        ...
        data = (
            pd.DataFrame({"ts": ts, "production_kwh": prod})
            .dropna(subset=["ts"])
            .copy()
        )
        ...
        rows_per_day = (
            data.groupby(norm_days).size() if not data.empty else pd.Series(dtype=int)
        )
        ...
        multi_row_days_ratio = (
            (rows_per_day >= 2).mean() if not rows_per_day.empty else 0
        )
        is_hourly = (
            (median_rows_per_day >= 2)
            or (avg_rows_per_day >= 2.0)
            or (p75_rows_per_day >= 2)
            or (multi_row_days_ratio >= 0.25)
        )
        if is_hourly:
            idx = data["ts"].dt.floor("h")
            out = (
                pd.DataFrame(
                    {"dt": idx, "production_kwh": data["production_kwh"].values}
                )
                .groupby("dt")
            ...
            gran = "hourly"
            logger.info(
                "Loaded %d production hours from %s to %s",
                len(out),
            ...
            out = (
                pd.DataFrame(
                    {"date": idx, "production_kwh": data["production_kwh"].values}
                )
                .groupby("date")
                .agg({"production_kwh": "sum"})
                .sort_index()
            ...
            gran = "daily"
            logger.info(
                "Loaded %d production days from %s to %s",
                len(out),
    ...
    def _looks_like_standard_schema(self, df: pd.DataFrame) -> bool:
        ...
        has_prod_kwh = any(
            (any(w in c for w in ["produktion", "production", "export"]))
            and ("kwh" in c)
            for c in cols
        )
    ...
    def _promote_header_if_present(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(max_scan):
            header_vals = [
                str(v).strip() if pd.notna(v) else "" for v in list(df.iloc[i].values)
            ]
    ...
    def _extract_table_from_grid(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out = (
            pd.DataFrame({"Date": dates, "Production kWh": prod})
            .dropna(subset=["Date"])
            .rename(columns={"Date": "date", "Production kWh": "production_kwh"})
        )
        return out

    def _parse_datetime_series(
        self, date_series: pd.Series, time_series: Optional[pd.Series] = None
    ) -> pd.Series:
        ...
        if pd.api.types.is_datetime64_any_dtype(s):
            ...
            if np.issubdtype(getattr(s, "dtype", object), np.number):
                ...
                ts_date = pd.to_datetime(
                    s_str, format="%Y-%m-%d", errors="coerce", utc=False
                )
                if ts_date.notna().mean() < 0.8:
                    ts_dt_try = pd.to_datetime(
                        s_str, format="%Y-%m-%d %H:%M", errors="coerce", utc=False
                    )
                    if ts_dt_try.notna().mean() >= 0.5:
                        ...
                        self.last_parse_format = (
                            self.last_parse_format or "auto_no_dayfirst"
                        )
                else:
        ...
        if not pd.api.types.is_datetime64_any_dtype(ts_date):
            ts_date = pd.to_datetime(
                ts_date.astype(str), errors="coerce", dayfirst=False
            )
        # 2) No time column
        ...
        if np.issubdtype(getattr(t, "dtype", object), np.number):
            ...
            hours_series = pd.Series(
                np.clip(np.round(hours), 0, 23), index=tnum.index
            ).astype("Int64")
        else:
            ...
            try:
                ...
                hours_series = (
                    pd.to_numeric(hh, errors="coerce")
                    .round()
                    .clip(0, 23)
                    .astype("Int64")
                )
